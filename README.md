...
The first-pass columns show the initial proof results. The after-feedback columns show the currently published proofs.
...
Within the first-pass totals, target claims were **45 / 60** checked, including **17 / 22** for DSToken; auxiliary proof claims were **1 / 10** checked. Complete initial candidate packages: **19 / 20**.

Both claim columns include target and auxiliary claims. DSToken has **22 / 22** target claims and **6 / 6** auxiliary `transfer` claims proved. Across the five projects, all **70 / 70** claims are proved.
...
[Metrics](METRICS.md) gives the token counts and estimated AI costs.
...
- [Verification status](STATUS.md): each selected function, its proof result, and its audit status.
- [Counting and cost](METRICS.md): claim counts, aggregate measurements, token use, and the AI-cost estimate.
- [Proof results](#first-pass-and-feedback-pass): the selected targets and their checked claims.
...
| Optimism L1 pausability | [Source](proofs/optimism-pause/proveWithdrawalTransaction/contract.sol) | [Packages](proofs/optimism-pause/) | [Status](STATUS.md) | [Metrics](METRICS.md) |
...
- `PROOF.md`: the proof results. Independent checks are in `audits/` where present.
